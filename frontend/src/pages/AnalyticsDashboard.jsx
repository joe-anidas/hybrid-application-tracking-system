...
export default function AnalyticsDashboard() {
  ...
  const calculateAnalytics = (apps, jobsList, usersList) => {
    ...
    const totalApplicants = Array.isArray(usersList)
      ? usersList.filter(
          (user) => user.role && user.role.toLowerCase() === "applicant",
        ).length
      : 0;
    ...
    apps.forEach((app) => {
      ...
      if (hoursDiff <= 24) {
        ...
        if (Object.prototype.hasOwnProperty.call(byHour, hourKey)) {
  ...
  const StatCard = ({ icon: _Icon, title, value, subtitle }) => (
    <div className="bg-white overflow-hidden shadow rounded-lg">
      <div className="p-5">
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <_Icon className="h-6 w-6 text-indigo-600" />
          </div>
  ...
  const TimeLineChart = () => {
    const getTimeData = () => {
      switch (timeFilter) {
        case "hour":
          return Object.entries(analytics.byHour || {}).map(
            ([time, value]) => ({
              time,
              applications: value,
            }),
        ...
        case "month":
          return Object.entries(analytics.byMonth || {}).map(
            ([time, value]) => ({
              time,
              applications: value,
            }),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Comprehensive application metrics and insights
          </p>
        </div>
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-sm text-gray-500">
                      {analytics.accepted} accepted out of {analytics.total}
                    </dd>
                  </dl>
                </div>
              </div>
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-medium text-gray-900">
                      {analytics.total > 0
                        ? (
                            (analytics.rejected / analytics.total) *
                            100
                          ).toFixed(1)
                        : 0}
                      %
                    ...
                    <dd className="text-sm text-gray-500">
                      {analytics.rejected} rejected applications
                    </dd>
                  </dl>
                </div>
              </div>
